...
const UpdateQualityReco = () => {
    ...
    const getRecoDetails = async (id) => {
        try {
            const { data } = await axios.get(`${process.env.REACT_APP_API}/admin/quality/recommendations/${id}`, config);
    ...
    const submitHandler = (e) => {
        ...
        console.log(recoDetails.id);
        updateReco(recoDetails.id, recoData);
    };
            ...
            <div className="flex">
                ...
                <div className="flex-1 py-10">
                    <div className="ml-[21rem] mr-20">
                        <h1 className="text-2xl font-bold mb-4 text-gray-900" style={{ fontSize: "30px", color: "#4AA032", fontFamily: "League Spartan", }}>Update Quality Recommendation</h1>
                        <form onSubmit={submitHandler} className="space-y-4" encType="multipart/form-data">
                            ...
                            >
                                ...
                                {valueOptions.map((option, index) => (
                                    <option key={index} value={option}>{factorOptions[recoDetails.factor].labels[index]}</option>
                                ))}
                            </select>
                            ...
                            <div className="col-span-full">
                                ...
                                <div className="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10">
                                    ...
                                    {oldImages && oldImages.map(img => (
                                        <img key={img} src={img.url} alt={img.url} className="ml-2 mt-3 mr-2" width="80px"/>
                                    ))}
                                    {imagesPreview.map(img => (
                                        <img src={img} key={img} alt="Images Preview" className="ml-2 mt-3 mr-2" width="80px"/>
                                    ))}
                                </div>
                            </div>
                            <Button
