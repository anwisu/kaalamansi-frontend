    ...
    const getRecoDetails = async (id) => {
        try {
            const { data } = await axios.get(
                `${process.env.REACT_APP_API}/admin/disease/recommendations/${id}`,
                config
            );
            console.log("Response:", data);

            setFactor(data.diseaseReco.factor);
            setValue(data.diseaseReco.value);
            setRecommendation(data.diseaseReco.recommendation);
            setImagePreview(data.diseaseReco.image.url);
        } catch (error) {
            console.error(error);
...
export default UpdateDiseaseReco